...
if SERVER then
        ...
        local mysql_username = 'ssrp' -- Your MySQL username.
        local mysql_password = 'siduck' -- Your MySQL password.
        local mysql_database = 'ssrp' -- Your MySQL database.
        ...
        function SSRP.GetMoneyForPlayer( ply )
				...
				SSRP.Query("SELECT bank FROM `players` WHERE steamid = '" .. ply:SteamID() .. "'", function(data)
                        if data and data[1] then	-- if the player already has monneys?
                                ply:SetNWInt("bank", data[1].bank)
                        else
                                ply:SetNWInt("bank", 0)
                        end
                end)
        ...
        function SSRP.CreateTable( ply, command, args )
                SSRP.Query("CREATE TABLE IF NOT EXISTS `players` ( `steamid` varchar(30) NOT NULL, `name` varchar(255) NOT NULL, `money` int(11) DEFAULT 0, `bank` int(11) DEFAULT 0, `job` int(2) NOT NULL DEFAULT 1, PRIMARY KEY (`steamid`) )", function(data)
